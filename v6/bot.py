...
################# Global variables #################
BNBTokenAddress = Web3.to_checksum_address("0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c")
...
def get_balance_bnb():
        ...
        humanReadable = globalWeb3.from_wei(float(result['balance']),'ether')
...
class TokenMonitor (threading.Thread):
                        ...
                        if TEST_SELL:
                                ...
                                while(not self.stop):
                                        ...
                                        if self.price*self.balance <= (self.buyPrice*self.balance) * lowThresholdSell:
                                                if self.price*self.balance <= (self.buyPrice*self.balance) * lowThresholdSell:
                                                        LOGGER.info("self.price : {price}".format(price = str(self.price)))
                                                        LOGGER.info("self.buyPrice * lowThresholdSell : {low}".format(low = str((self.buyPrice*self.balance) * lowThresholdSell)))
        ...
        def getPair(self, tokenAddress):
                tokenAddress = self.web3Thread.to_checksum_address(tokenAddress)
        ...
        def getReserves(self, pairAddressforReserves):  # fundamental code for liquidity detection
                pairAddressforReserves = self.web3Thread.to_checksum_address(pairAddressforReserves)
        ...
        def buy(self):
                ...
                try:
                        if(self.tokenAddress != None):
                                ...
                                tokenToBuy = self.web3Thread.to_checksum_address(self.tokenAddress)
...
Adress : ".format(dollarz = str(round(snipeBNBAmount*BNBPrice, 2))) + self.tokenAddress)
                                        ...
                                        try:
                                                ...
                                                owner = self.web3Thread.to_checksum_address(walletAddress)
                                                spender = self.web3Thread.to_checksum_address(pancakeSwapRouterAddress)
        ...
        def sell(self):
                ...
                contract_id = self.web3Thread.to_checksum_address(self.tokenAddress)
...
async def new_message_listener(event):
        ...
        regexFirstAlert = r"Solid profit expected, buy ASAP"
